...
def get_parameters_string(custom_tool_definition) -> str:
    return json.dumps(custom_tool_definition, indent=1, ensure_ascii=False)

def get_system_content(tools):
    custom_tools = ""
    for t in tools:
        custom_tools += get_instruction_string(t) + "\n"
        custom_tools += get_parameters_string(t) + "\n\n"
    return SYSTEM_MESSAGE.format(custom_tools = custom_tools)
...
class PromptTemplate:
    ...
    def inject_system_messages_based_on_tools(
        ...
        messages_clone.insert(
            0, {"role": "system", "content": get_system_content(functions)}
        )
        if is_code_interpreter:
            messages_clone.insert(1, {"role": "system", "content": PYTHON_RUN_SYS_MSG})
        else:
            messages_clone.insert(1, {"role": "system", "content": INSTRUCT_MESSAGE})
